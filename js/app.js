...
async function renderPatternList() {
  ...
  container.innerHTML = patternData
    .map((p) => {
...
function renderPatternInfo() {
  ...
  if (p.sizes && p.sizes.length > 0) {
    sizeSelect.innerHTML = p.sizes.map((s, i) => `<option value="${i}">${s.name}</option>`).join("");
...
function renderStep() {
  ...
  document.getElementById("next-btn").textContent = currentStepIndex === flattenedSteps.length - 1 ? "Finish" : "Next";
